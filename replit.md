...
- **api-server** (`/api`) — Express 5. Routes: `GET /api/healthz`, `POST /api/chat`, `GET /api/preach/song`.
  ...
  - **Song picker** (`routes/preach.ts`) is fully dynamic — no curated track list. Picks a random artist from `ARTISTS` (~150 across hip-hop, pop, classic rock, soul, indie, electronic, country, jazz, etc.), fetches that artist's catalog of synced lyrics from `lrclib.net/api/search?q=<artist>` (loose-normalized artist filter to drop "feat." mismatches), picks a random track, then resolves a YouTube videoId by scraping `youtube.com/results?search_query=<artist+title+audio>` (no API key — sets `Cookie: CONSENT=YES+1` to skip the EU consent wall). Resolved `videoId`s are cached in-process keyed by `<norm(artist)>::<norm(title)>` so repeat plays skip the YouTube round-trip. Up to 6 artist attempts × 4 tracks each before giving up with a 503.
