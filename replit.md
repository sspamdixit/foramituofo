...
- **buddha-chat** (`/`) — Monk Radio single-page web app (monkradio.xyz). Music-discovery first: a song auto-plays on load and Buddha sings/hums the lyrics. The cartoon Buddha sprite (PNGs in `artifacts/buddha-chat/public/`: `idle`, `thinking`, `speaking`, `blessing`, `refusing`, plus `bg-tile.png`, `fonts/Pencil.ttf`) is the centered visual anchor.
  - **Music is always on**: `usePreachSong(true)` is wired permanently in `home.tsx`. The `PreachPlayer` is pinned to the bottom and an HTML5 `<audio>` element streams audio from `/api/preach/stream/:videoId`. Songs auto-advance on end / on error.
...
- **api-server** (`/api`) — Express 5. Routes: `GET /api/healthz`, `POST /api/chat`, `GET /api/preach/song`, `GET /api/preach/stream/:videoId`.
  - **Chat** uses Replit AI Integrations for Gemini (`AI_INTEGRATIONS_GEMINI_API_KEY` + `AI_INTEGRATIONS_GEMINI_BASE_URL`, auto-provisioned — no user-supplied key) via the `@google/genai` SDK, model chain `gemini-2.5-flash` → `gemini-2.5-flash-lite`. Streams as SSE (`{type:"delta",text}`, `{type:"done"}`, `{type:"refused"}`, `{type:"error"}`).
  - **Song picker** (`routes/preach.ts`): picks a random artist from `ARTISTS` (~150 across genres), fetches that artist's synced lyrics from `lrclib.net`, picks a random track, then resolves a YouTube videoId + duration via Lavalink public nodes (`lavalink.devamop.in`, `lava.link`, etc.). videoId+duration are cached in-process.
  - **Audio streaming** (`routes/preach.ts` → `/api/preach/stream/:videoId`): resolves the direct YouTube CDN URL via `yt-dlp` (Python, installed at `.pythonlibs/bin/yt-dlp`, uses the ANDROID_VR client which needs no PO Token). The CDN URL is cached 4 h. Requests are proxied with full byte-range support (`Accept-Ranges: bytes`, `206 Partial Content`) so the browser `<audio>` element gets native seek support. `/preach/song` pre-warms the cache fire-and-forget so the URL is ready by the time the user clicks play.
