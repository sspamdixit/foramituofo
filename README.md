...
- **Lyrics** — pulled from [lrclib.net](https://lrclib.net) as standard LRC. A content-aware sync algorithm shows each line from its timestamp until either the next line or an estimated singing duration (so short interjections clear quickly and long held finals stay visible). Per-track manual sync nudges are saved in localStorage.
